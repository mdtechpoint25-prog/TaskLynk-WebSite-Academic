...
export default function CancelledPage() {
  ...
  const filteredJobs = useMemo(() => {
    return jobs.filter(j => j.status === 'cancelled' || j.status === 'canceled');
  }, [jobs]);
  ...
  return (
    <main className="bg-background">
      <div className="p-3 md:p-4 lg:p-5 w-full">
          <div className="mb-8">
            <h1 className="text-2xl font-bold mb-2">Cancelled</h1>
            <p className="text-sm text-muted-foreground">
              View orders that have been cancelled by admin or client. Access status notes or reasons for cancellation.
            </p>
          </div>
