...
import { Pause, FileText, Search } from 'lucide-react';
...
export default function OnHoldPage() {
  ...
  const filteredJobs = useMemo(() => {
    return jobs.filter(j => j.status === 'on_hold' || j.status === 'on-hold');
  }, [jobs]);
    ...
    <div className="min-h-screen flex bg-background">
      ...
      <main className="flex-1 pt-[72px] ml-0 md:ml-64 bg-background transition-all duration-300">
        <div className="p-3 md:p-4 lg:p-5 w-full">
          <div className="mb-8">
            <h1 className="text-2xl font-bold mb-2">On Hold</h1>
            <p className="text-sm text-muted-foreground">
              View orders temporarily paused or awaiting clarification from admin or client before proceeding.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">On Hold Orders</CardTitle>
