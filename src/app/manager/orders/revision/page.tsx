...
export default function ManagerRevisionOrdersPage() {
  ...
  const fetchJobs = async () => {
    try {
      ...
      const response = await fetch(`/api/manager/orders?managerId=${user?.id}&status=revision`, {
      ...
      if (response.ok) {
        ...
        toast.error('Failed to load revision orders');
      }
    } catch (error) {
  ...
  const handleExport = async () => {
    ...
    try {
      const token = localStorage.getItem('bearer_token');
      const res = await fetch(`/api/manager/orders?managerId=${user.id}&status=revision&format=csv`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (!res.ok) {
      ...
      a.href = url;
      a.download = `manager-revision-orders-${user.id}.csv`;
    ...
    <div className="w-full">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">
          Orders requiring revisions and improvements
        </p>
      </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="flex items-center gap-2">
                <Badge className="bg-orange-500 rounded-xl">
                  {jobs.length}
                </Badge>
                Orders Needing Revision
              </CardTitle>
              <CardDescription>
                Review and manage revision requests
              </CardDescription>
            </div>
        ...
        <CardContent>
          {loadingJobs ? (
            ...
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p className="text-lg font-medium mb-2">No Revision Orders</p>
              <p className="text-sm">Orders needing revisions will appear here</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <Table>
                ...
                <TableBody>
                  {jobs.map((job) => (
                    <TableRow key={job.id} className="bg-orange-50 dark:bg-orange-950/20 hover:bg-orange-100 dark:hover:bg-orange-950/30 cursor-pointer transition-colors" onClick={() => router.push(`/manager/jobs/${job.id}`)}>
