...
import { Eye, Clock, CheckCircle2, Download } from 'lucide-react';
...
export default function ManagerCompletedOrdersPage() {
  ...
  const fetchJobs = async () => {
    try {
      ...
      const response = await fetch(`/api/manager/orders?managerId=${user?.id}&status=completed`, {
  ...
  const handleExport = async () => {
    ...
    try {
      const token = localStorage.getItem('bearer_token');
      const res = await fetch(`/api/manager/orders?managerId=${user.id}&status=completed&format=csv`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (!res.ok) {
      ...
      a.href = url;
      a.download = `manager-completed-orders-${user.id}.csv`;
          ...
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="flex items-center gap-2">
                <Badge className="bg-green-600 rounded-xl">{jobs.length}</Badge>
                Completed Orders
              </CardTitle>
              <CardDescription>
        ...
        <CardContent>
          {loadingJobs ? (
            ...
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p className="text-sm">Finished orders will appear here</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <Table>
                ...
                <TableBody>
                  {jobs.map((job) => (
                    <TableRow key={job.id} className="bg-green-50 dark:bg-green-950/20 hover:bg-green-100 dark:hover:bg-green-950/30 transition-colors cursor-pointer" onClick={() => router.push(`/manager/jobs/${job.id}`)}>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-1 text-xs text-muted-foreground">
                          ...
                          {format(new Date(job.completedAt || job.createdAt), 'MMM dd, yyyy')}
                        </div>
                      </TableCell>
