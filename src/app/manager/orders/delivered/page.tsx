...
import { Eye, Clock, PackageCheck, Download } from 'lucide-react';
...
export default function ManagerDeliveredOrdersPage() {
  ...
  const fetchJobs = async () => {
    try {
      ...
      const response = await fetch(`/api/manager/orders?managerId=${user?.id}&status=delivered`, {
  ...
  const handleExport = async () => {
    ...
    try {
      const token = localStorage.getItem('bearer_token');
      const res = await fetch(`/api/manager/orders?managerId=${user.id}&status=delivered&format=csv`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (!res.ok) {
      ...
      a.href = url;
      a.download = `manager-delivered-orders-${user.id}.csv`;
    ...
    <div className="w-full">
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2 flex items-center gap-2">
          <PackageCheck className="w-8 h-8 text-cyan-600" />
          Delivered Orders
        </h1>
        <p className="text-muted-foreground">
          Orders that have been delivered and awaiting client approval
        </p>
      </div>
      ...
      <Card className="rounded-2xl">
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle>Delivered Orders ({jobs.length})</CardTitle>
              <CardDescription>
                Monitor delivered orders and track client responses
              </CardDescription>
            </div>
        ...
        <CardContent>
          {loadingJobs ? (
            ...
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p className="text-sm">Delivered orders awaiting client approval will appear here</p>
            </div>
          ) : (
