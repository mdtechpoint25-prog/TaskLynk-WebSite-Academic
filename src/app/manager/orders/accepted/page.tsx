...
import { Eye, Clock, CheckCircle, Download } from 'lucide-react';
...
export default function ManagerAcceptedOrdersPage() {
  ...
  const fetchJobs = async () => {
    try {
      ...
      const response = await fetch(`/api/manager/orders?managerId=${user?.id}&status=accepted`, {
  ...
  const handleExport = async () => {
    ...
    try {
      const token = localStorage.getItem('bearer_token');
      const res = await fetch(`/api/manager/orders?managerId=${user.id}&status=accepted&format=csv`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (!res.ok) {
      ...
      a.href = url;
      a.download = `manager-accepted-orders-${user.id}.csv`;
    ...
    <div className="w-full">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">
          Orders accepted by clients and awaiting payment confirmation
        </p>
      </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="flex items-center gap-2">
                Accepted Orders ({jobs.length})
              </CardTitle>
              <CardDescription>
                Client-accepted orders pending payment confirmation
              </CardDescription>
            </div>
        ...
        <CardContent>
          {loadingJobs ? (
            ...
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p className="text-lg font-medium mb-2">No Accepted Orders</p>
              <p className="text-sm">Client-approved orders will appear here</p>
            </div>
          ) : (
