...
import { Eye, Clock, UserCheck, Download } from 'lucide-react';
...
export default function ManagerAssignedOrdersPage() {
  ...
  const fetchJobs = async () => {
    try {
      ...
      const response = await fetch(`/api/manager/orders?managerId=${user?.id}&status=assigned`, {
  ...
  const handleExport = async () => {
    ...
    try {
      const token = localStorage.getItem('bearer_token');
      const res = await fetch(`/api/manager/orders?managerId=${user.id}&status=assigned&format=csv`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (!res.ok) {
      ...
      a.href = url;
      a.download = `manager-assigned-orders-${user.id}.csv`;
    ...
    <div className="w-full">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">
          Orders assigned to writers and awaiting work to begin
        </p>
      </div>
      ...
      <Card className="rounded-2xl">
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle>Assigned Orders ({jobs.length})</CardTitle>
              <CardDescription>
                Monitor assigned orders and writer progress
              </CardDescription>
            </div>
        ...
        <CardContent>
          {loadingJobs ? (
            ...
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p className="text-lg font-medium mb-2">No Assigned Orders</p>
              <p className="text-sm">Orders will appear here once assigned to writers</p>
            </div>
          ) : (
                ...
                <TableBody>
                  {jobs.map((job) => (
                    ...
                    >
                      ...
                      <TableCell>
                        <Badge className="bg-blue-500 rounded-xl">{job.writer?.name || 'Unassigned'}</Badge>
                      </TableCell>
                      <TableCell>
