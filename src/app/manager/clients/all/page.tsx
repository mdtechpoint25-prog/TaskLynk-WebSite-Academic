...
import { Eye, Mail, Phone, Calendar, Users } from 'lucide-react';
...
export default function ManagerAllClientsPage() {
  ...
  const fetchClients = async () => {
    try {
      ...
      const response = await fetch(`/api/manager/clients?managerId=${user?.id}`, {
    ...
    <div className="w-full">
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground">
          View and manage clients assigned to your manager account
        </p>
      </div>

      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base md:text-lg">Assigned Clients ({clients.length})</CardTitle>
          <CardDescription className="text-xs md:text-sm">
            Complete list of your assigned clients
          </CardDescription>
        </CardHeader>
        <CardContent>
          {loadingClients ? (
                    ...
                    <TableRow key={client.id}>
                      ...
                      <TableCell>
                        {client.role === 'account_owner' ? (
                          <Badge className="bg-purple-500 text-[10px] px-2 py-0.5">Account Owner</Badge>
                        ) : (
                          <Badge variant="outline" className="text-[10px] px-2 py-0.5">Regular</Badge>
                        )}
                      </TableCell>
                      <TableCell>
                        {client.approved ? (
