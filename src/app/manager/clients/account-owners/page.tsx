...
import { Eye, Mail, Phone, Calendar, Crown } from 'lucide-react';
...
export default function ManagerAccountOwnersPage() {
  ...
  const fetchClients = async () => {
    try {
      ...
      const response = await fetch(`/api/manager/clients?managerId=${user?.id}&accountOwner=true`, {
        ...
        <div className="p-3 md:p-4 lg:p-5 w-full">
          <div className="mb-6">
            ...
            <p className="text-sm text-muted-foreground">
              Premium clients assigned to your manager account
            </p>
          </div>
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-base md:text-lg flex items-center gap-2">
                <Badge className="bg-purple-500 text-[10px] px-2 py-0.5">{clients.length}</Badge>
                Assigned Account Owners
              </CardTitle>
              <CardDescription className="text-xs md:text-sm">
                Clients with special permissions and privileges
              </CardDescription>
            </CardHeader>
            <CardContent>
              {loadingClients ? (
                ...
                <div className="overflow-x-auto">
                  <Table>
                    ...
                    <TableBody>
                      {clients.map((client) => (
                        <TableRow key={client.id} className="bg-purple-50 dark:bg-purple-950/20">
